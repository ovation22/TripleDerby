...
/// </summary>
public class RabbitMqBrokerAdapter(ILogger<RabbitMqBrokerAdapter> logger) : IMessageBrokerAdapter
{
    ...
    public async Task ConnectAsync(MessageBrokerConfig config, CancellationToken cancellationToken)
    {
        ...
        var exchange = config.ProviderSpecific.GetValueOrDefault("Exchange", "triplederby.events");
        ...
        consumer.ReceivedAsync += async (sender, ea) =>
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    logger.LogWarning(
                        "Message processing failed (Queue: {Queue}, DeliveryTag: {DeliveryTag}, Requeue: {Requeue}, Reason: {Reason})",
                        _queueName, ea.DeliveryTag, result.Requeue, result.ErrorReason);
                }
            }
