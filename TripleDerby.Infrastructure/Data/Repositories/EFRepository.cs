...
public abstract class EFRepository(DbContext dbContext, ILogger<EFRepository> logger)
    : IEFRepository
{
    /// <inheritdoc />
    ...
    /// Uses the repository's <see cref="dbContext"/> so repository methods called
    /// inside the operation will participate in the same transaction.
    /// </summary>
    public virtual async Task ExecuteInTransactionAsync(Func<Task> operation, CancellationToken cancellationToken = default)
    {
        ...
        // Begin transaction on the underlying DbContext
        ...
        catch
        {
            ...
            {
                logger.LogError(ex, "Failed to rollback transaction after operation failure.");
            }
